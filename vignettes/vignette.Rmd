...
Data from ORFquant can also be utilised. RNA-Seq coverage previously calculated with `bedtools coverage` and the P-sites calculated by ORFquant can be merged with the `inspectorORF::merge_tracks_with_ORFquant` function as follows, which will return a GRanges.
...
Again, although reads are within the highlighted codsing sequence, such reads are low, making it unlikely that this coding sequence is truly translated.
...
The rest of the transcript can also be plot by setting `plot_region = c(-1, -1)` which will plot these regions are 5'UTR and 3'UTR, essentially the same as `inspectorORF::transcript_plot` with a region highlighted.
...
This plot shows that coverage for both RNA-Seq and P-Sites are significantly higher and spread across the coding sequence of interested in our treated (differentiated) cells. Suggessting the coding sequence is translated.
