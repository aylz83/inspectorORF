...
Detecting translated open reading frames (ORFs) within Ribo-Seq data often requires evaluating the reads within your captured ORFs to determine if typical properties of translation, such as triplet periodicity are present. inspectorORF aims to simplify the task of plotting ORFs, with the following features:
 ...
 - Plotting of individual exons
 - Plotting of exons and introns (if intronic data is available)
 - Annotating codons of interest such as in-frame or out of frame, non-canonical, up/downstream start codons and stop codons
...
Alternative coverage tools can also be used, as long as the resulting bed files consists of chromosome, start, end, strand and score columns, where each start and end position is incremental, or 
chromosome, start, end, [ignored], [ignored], strand, position, score where position is incremental. Columns must be in those orders
...
```{r tx_tracks_from_genome_tracks}
...
```{r first_tx_from_gene_plot, fig.width = 8, fig.height = 4}
inspectorORF::transcript_plot(
	tx_tracks_from_gene,
	transcript_filter = "ENST00000343702.9" # Requires 1 transcript at a time
)
...
Exons making up a transcript can also be plot by splitting the figure into exons with the `split_exons = TRUE` option -

```{r exons_tx_plot, fig.width = 10, fig.height = 4}
inspectorORF::transcript_plot(
	tx_tracks,
	transcript_filter = "ENST00000343702.9", # Requires 1 transcript at a time
	split_exons = TRUE
...
The annotated coding sequence within ENST00000343702.9 can be plot to get a better understanding the P-Site reads we do have, to see if they are primarily within the coding sequence. This can be undertaken by highlighting a region of interest by specifying a `start_position` and `stop_position`.
...
An additional triplet periodicity summary plot for the remainder of the transcript can be enabled with the `summarise_outside_region = TRUE`:
...
### Retaining reads within introns for plotting

During gene or transcript track generation, reads within introns can be retained with the `keep_introns = TRUE` to enable plotting intronic reads later -

```{r intron_read_retention}
...
Including and annotating introns within a plot can then be performed by setting the `split_exons` parameter of transcript_plot to "with_introns" -

```{r intron_read_retention_plot, fig.width = 10, fig.height = 4}
inspectorORF::transcript_plot(
	my_tx_tracks,
	transcript_filter = "ENST00000343702.9", # Requires 1 transcript at a time
...
```{r orf_plot_multiple_conditions, fig.width = 10, fig.height = 6}
inspectorORF::orf_plot(
	tx_tracks,
	transcript_filter = "ENST00000343702.9",
	start_position = 456,
...
```{r plot_additional_tx_data, fig.width = 10, fig.height = 8}
...
**PLEASE NOTE** - Newer versions of ggplot (> 4.0) can cause the generation of some warnings when using `codon_queries` due to a mix of packages inspectorORF uses.
These are related to faceting dimension calculations and do not impact the final plot.
...
```{r orf_plot_framing_codons, fig.width = 10, fig.height = 8}
#| warning: false
#| message: false
...
```{r orf_plot_framing_codons_tx, fig.width = 10, fig.height = 8}
#| warning: false
#| message: false
...
When annotating codons, stop codons can also be included with `annotate_stop = TRUE`. This can be particularly useful when annotating in-frame start codons across the transcript to indicate alternative ORFs which are possibly translated.

```{r orf_plot_annotate_stop, fig.width = 10, fig.height = 8}
#| warning: false
#| message: false
...
By default, `annotate_stop = TRUE` will search for all three possible stop codons (UAA, UAG and UGA). `annotate_stop` also accepts a character vector to search for specific stop codons instead -

```{r orf_plot_stop_alt, fig.width = 10, fig.height = 8}
#| warning: false
#| message: false
...
Annotated codons can also be plot on their on track facet, which is particularly useful in the case of lots of codons. This can be enabled by naming the codon query with `annotation_label = "My Codons"`.

By default, all annotated codons will be within the same track facet titled 'Labelled Codons'

```{r orf_plot_labelled_codons, fig.width = 10, fig.height = 8}
#| warning: false
#| message: false
